...
export default function Airtime() {
  ...
  const buy = async (e) => {
    ...
    try {
      ...
      setPurchaseResult({
        ok: true,
        reference: res.reference || `MELE-AIRTIME-${Date.now()}`,
      ...
      if (isUncertainPurchaseError(message)) {
        setPurchaseResult({
          ok: true,
          reference: `MELE-AIRTIME-PENDING-${Date.now()}`,
          status: "pending",
        ...
        setPurchaseResult({
          ok: false,
          reference: `MELE-AIRTIME-ATTEMPT-${Date.now()}`,
          status: "failed",
          created_at: new Date().toISOString(),
          network: payload.network,
          phone: payload.phone_number,
  ...
  const downloadReceipt = async () => {
    ...
    try {
      ...
      pdf.save(`meledata-airtime-receipt-${safeReference}.pdf`);
  ...
  const shareReceipt = async () => {
    ...
    try {
      ...
      const result = await shareReceiptCapture({
        ...
        title: "MELE DATA Airtime Receipt",
        text: shareText(),
        fileName: `meledata-airtime-receipt-${safeReference}.png`,
      });
      if (!result.ok) {
