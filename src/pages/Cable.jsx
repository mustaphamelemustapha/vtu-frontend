...
export default function Cable() {
  ...
  const buy = async (e) => {
    ...
    try {
      ...
      setPurchaseResult({
        ok: true,
        reference: res.reference || `MELE-CABLE-${Date.now()}`,
      ...
      setPurchaseResult({
        ok: false,
        reference: `MELE-CABLE-ATTEMPT-${Date.now()}`,
        status: "failed",
  ...
  const downloadReceipt = async () => {
    ...
    try {
      ...
      pdf.save(`meledata-cable-receipt-${safeReference}.pdf`);
  ...
  const shareReceipt = async () => {
    ...
    try {
      ...
      const result = await shareReceiptCapture({
        ...
        title: "MELE DATA Cable Receipt",
        text: shareText(),
        fileName: `meledata-cable-receipt-${safeReference}.png`,
      });
      if (!result.ok) {
