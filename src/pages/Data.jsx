...
export default function Data() {
  ...
  const mapTransactionToResult = (tx, chosenPlan, fallbackPhone) => {
    ...
    return {
      ...
      reference: tx?.reference || `MELE-${Date.now()}`,
      status,
  ...
  const buy = async (planCode) => {
    ...
    try {
      ...
      const reference = res.reference || `MELE-${Date.now()}`;
      ...
      if (isTransientPurchaseError(err)) {
        ...
        setPurchaseResult({
          ok: true,
          reference: `MELE-PENDING-${Date.now()}`,
          status: "pending",
      ...
      setPurchaseResult({
        ok: false,
        reference: `MELE-ATTEMPT-${Date.now()}`,
        status: "failed",
        created_at: new Date().toISOString(),
        test_mode: false,
        plan: chosenPlan,
  ...
  const downloadReceipt = async () => {
    ...
    try {
      ...
      pdf.save(`meledata-data-receipt-${safeReference}.pdf`);
  ...
  const shareReceipt = async () => {
    ...
    const result = await shareReceiptCapture({
      ...
      title: "MELE DATA Data Receipt",
      text: shareText(),
      fileName: `meledata-data-receipt-${safeReference}.png`,
    });
    if (!result.ok) {
            ...
            <div className="data-result-receipt">
              ...
              <div className="data-result-receipt-rows">
                ...
                <div className="data-result-row">
                  ...
                  <strong>{profile?.full_name || profile?.email || "MELE DATA User"}</strong>
                </div>
                <div className="data-result-row">
                  <span>Provider</span>
          ...
          <div className="receipt-capture-layer" aria-hidden="true">
            <div className="receipt-sheet" ref={receiptCaptureRef}>
              <div className="receipt-sheet-header">
                <img src="/brand/axisvtu-logo.png" alt="MELE DATA" />
                <div>
                  <div className="label">MELE DATA Purchase Receipt</div>
                  <h4>Data Purchase</h4>
                </div>
              ...
              <div className="receipt-sheet-footer">
                <div>Generated by MELE DATA</div>
