...
export default function Exam() {
  ...
  const buy = async (e) => {
    ...
    try {
      ...
      setPurchaseResult({
        ok: true,
        reference: res.reference || `MELE-EXAM-${Date.now()}`,
      ...
      setPurchaseResult({
        ok: false,
        reference: `MELE-EXAM-ATTEMPT-${Date.now()}`,
        status: "failed",
  ...
  const downloadReceipt = async () => {
    ...
    try {
      ...
      pdf.save(`meledata-exam-receipt-${safeReference}.pdf`);
  ...
  const shareReceipt = async () => {
    ...
    try {
      ...
      const result = await shareReceiptCapture({
        ...
        title: "MELE DATA Exam PIN Receipt",
        text: shareText(),
        fileName: `meledata-exam-receipt-${safeReference}.png`,
      });
      if (!result.ok) {
