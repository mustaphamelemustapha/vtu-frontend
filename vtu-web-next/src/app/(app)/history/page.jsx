    ...
    <div className="space-y-6 pb-8">
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {loading ? <div className="text-sm text-slate-700">Loading history...</div> : null}
          {transactions.length === 0 && !loading ? <div className="text-sm text-slate-700">No transactions yet.</div> : null}
          {transactions.map((tx) => (
            <div key={tx.reference || tx.id} className="flex items-center justify-between gap-4 rounded-2xl border border-slate-300 bg-[#fcfbf8] p-4">
              <div>
                ...
                <div className="mt-1 text-xs text-slate-700">{tx.reference || '—'} • {formatDateTime(tx.created_at)}</div>
              </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {reports.length === 0 ? <div className="text-sm text-slate-700">No support reports yet.</div> : null}
          {reports.map((report) => (
            <div key={report.id} className="rounded-2xl border border-slate-300 bg-[#fcfbf8] p-4">
              ...
              <div className="mt-2 text-sm text-slate-700">{report.message || report.summary || 'Support report details available in backend.'}</div>
            </div>
          ))}
        </CardContent>
      </Card>
