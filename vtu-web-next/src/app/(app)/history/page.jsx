    ...
    <div className="space-y-6 pb-8">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {[
          ...
          <Card key={item.label}><CardContent className="p-5"><div className="axis-label">{item.label}</div><div className="mt-2 text-2xl font-semibold text-white">{item.value}</div></CardContent></Card>
        ))}
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          {transactions.length === 0 && !loading ? <div className="text-sm text-slate-400">No transactions yet.</div> : null}
          {transactions.map((tx) => (
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {reports.length === 0 ? <div className="text-sm text-slate-400">No support reports yet.</div> : null}
          {reports.map((report) => (
            <div key={report.id} className="rounded-2xl border border-white/8 bg-white/4 p-4">
              <div className="flex items-center justify-between gap-3">
                <div className="text-sm font-medium text-white">Report #{report.id}</div>
                <Badge tone={String(report.status || '').toLowerCase() === 'resolved' ? 'success' : 'warning'}>{report.status || 'open'}</Badge>
              </div>
              <div className="mt-2 text-sm text-slate-400">{report.message || report.summary || 'Support report details available in backend.'}</div>
            </div>
          ))}
        </CardContent>
      </Card>
