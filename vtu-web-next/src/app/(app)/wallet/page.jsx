    ...
    <div className="space-y-6 pb-8">
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_360px]">
        <Card>
          ...
          <CardContent>
            <div className="flex items-center justify-between rounded-3xl border border-slate-300 bg-[#fcfbf8] p-5">
              <div>
                ...
                <div className="mt-2 text-sm text-slate-700">{loading ? 'Syncing wallet data...' : 'Wallet loaded from the existing API.'}</div>
              </div>
          ...
          <CardContent className="space-y-4">
            {primary ? (
              <div className="rounded-3xl border border-slate-300 bg-[#fcfbf8] p-4">
                ...
                <div className="mt-2 text-sm text-slate-700">{primary.account_name || 'AxisVTU Wallet'}</div>
              ...
              <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-4 text-sm text-slate-700">Dedicated accounts will appear here once generated.</div>
            )}
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {ledger.length === 0 ? <div className="text-sm text-slate-700">No ledger records yet.</div> : null}
          {ledger.map((item) => (
            <div key={item.id || item.reference} className="flex items-center justify-between gap-4 rounded-2xl border border-slate-300 bg-[#fcfbf8] p-4">
              <div>
                <div className="text-sm font-medium text-slate-950">{item.description || item.reference || 'Wallet entry'}</div>
                <div className="text-xs text-slate-700">{formatDateTime(item.created_at)}</div>
              </div>
