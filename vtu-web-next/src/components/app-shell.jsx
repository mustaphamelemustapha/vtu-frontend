    ...
    <div className="axis-shell grid min-h-screen lg:grid-cols-[280px_minmax(0,1fr)]">
      ...
      <main className="min-w-0 bg-[#f8f5ef]">
        <header
          className={cn(
            ...
            mobileChromeDark
              ? 'border-white/10 bg-[#0b0f14]/94 md:border-slate-200 md:bg-white/90'
        ...
        >
          <div className="flex items-center gap-2 px-3 py-2.5 md:px-6 md:py-3 lg:px-8">
            ...
            <div className="min-w-0 flex-1 md:hidden">
              ...
              <div className={cn('truncate text-[11px]', mobileChromeDark ? 'text-white/45' : 'text-slate-500')}>
                AxisVTU
              </div>
            </div>
          ...
          <nav
            className={cn(
              ...
              mobileChromeDark ? 'border-white/10' : 'border-slate-200'
            )}
          >
            {appNav.map((item) => {
              const Icon = item.icon;
              ...
              return (
                ...
                >
                  <Icon className={cn('h-3 w-3', active ? 'text-orange-500' : mobileChromeDark ? 'text-white/42' : 'text-slate-400')} />
                  <span>{label}</span>
                </Link>
              );
            })}
      ...
      {mobileMenuOpen ? (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          />
          <aside className="relative flex h-full w-[82vw] max-w-[340px] flex-col border-r border-white/10 bg-[#0b0f14] p-4 text-white shadow-2xl">
            <div className="flex items-center justify-between gap-3">
              <Link href="/dashboard" className="flex min-w-0 items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.04] px-3 py-3">
                ...
                <div className="min-w-0">
                  ...
                  <div className="truncate text-xs text-white/45">Mobile workspace</div>
                </div>
              </Link>
              <Button
                variant="secondary"
                size="icon"
                className="h-9 w-9 shrink-0 rounded-xl border-white/10 bg-white/[0.04] text-white hover:bg-white/[0.08]"
                onClick={() => setMobileMenuOpen(false)}
            ...
            <nav className="mt-6 space-y-1.5">
              {appNav.map((item) => {
                ...
                return (
                  ...
                  >
                    <Icon className={cn('h-4 w-4', active ? 'text-orange-400' : 'text-white/40')} />
                    <span>{item.label}</span>
                  </Link>
                );
              })}
            ...
            <div className="mt-auto space-y-4 rounded-3xl border border-white/10 bg-white/[0.04] p-4">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <div className="truncate text-xs text-white/45">{profile.email || 'No email attached'}</div>
                </div>
              </div>
              <Button
                variant="secondary"
